...
from tensorflow.keras.layers import Input, Conv2D, ReLU, BatchNormalization, \
    Flatten, Dense, Reshape, Conv2DTranspose, Activation, Lambda, Embedding, concatenate, LeakyReLU, Dropout
...
class VAE:
    ...
    def _build_autoencoder(self):
        ...
        model_output = self.decoder(self.encoder(model_input)[0])
    ...
    def _add_first_conv(self, x):
        ...
        x = LeakyReLU(name=f"decoder_relu_0")(x)
    ...
    def _add_conv_transpose_layer(self, layer_index, x):
        ...
        x = LeakyReLU(name=f"decoder_relu_{layer_num}")(x)
        return x
    ...
    def _add_bottleneck(self, x, y):
        ...
        self.mu = Dense(self.latent_space_dim, name="mu")(latent_space)
        self.log_variance = Dense(self.latent_space_dim,
                                  name="log_variance")(latent_space)

        z = SamplingLayer()([self.mu, self.log_variance])

        return z, self.mu, self.log_variance


if __name__ == "__main__":
    tiny_vae = VAE(
        input_shape=(144, 304, 2),
        inf_vector_shape=(2, 16),
        conv_filters=(64, 128, 256, 512),
        conv_kernels=(6, 6, 6, 6),
