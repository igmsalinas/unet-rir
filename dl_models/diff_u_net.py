...
class DiffUNet:
    ...
    def _build(self):
        ...
        concat_out = Add()([encoding_5_out, vector_out])
        # ----- DECODING -----
        ...
        out = tf.keras.layers.Conv2D(2, (1, 1), activation='linear')(x)

        self.model = tf.keras.Model(self._model_input, out, name="U-Net")

    def vector_block(self, input_v, block_shape):
        shape = block_shape[1:]
        dim = np.prod(shape)

        features = Embedding(1500, 128)(input_v)
        f_features = Flatten()(features)
        x = Dense(dim, name="encoder_inf_dense")(f_features)
        x = Dropout(.5)(x)
        x = Reshape(shape)(x)
        return x
...
if __name__ == "__main__":
    unet = UNet(input_shape=(144, 128, 2),
