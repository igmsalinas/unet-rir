...
class Trainer:
    ...
    def __init__(self, alpha, n_epochs, optimizer, callbacks, lr_exp_decay, lr0, file_name):
        ...
        if 'nadam' in optimizer:
            ...
            self.optimizer = tfa.optimizers.LAMB(learning_rate=self.lr0)
    ...
    def train(self, model, train_generator, val_generator):
        ...
        for epoch in range(0, self.n_epochs):
            ...
            for i in range(0, numUpdates):
                ...
                spec_in, spec_out, emb = train_generator.__getitem__(i)
            ...
            for i in range(0, numUpdates_val):
                ...
                # We take from our data generator one batch
                spec_in, spec_out, emb = val_generator.__getitem__(i)
    ...
    def model_loss(self, y_true, y_pred):
        ...
        stft_true, phase_true = np.expand_dims(np.moveaxis(y_true, 3, 0), -1)
        stft_pred, phase_pred = np.expand_dims(np.moveaxis(y_pred, 3, 0), -1)

        loss_stft = amplitude_loss(stft_true, stft_pred)
        loss_phase = phase_loss(phase_true, phase_pred)

        # loss = K.mean(loss_phase + loss_stft, axis=-1)
        loss = amplitude_loss(y_true, y_pred)
        return loss, loss_phase, loss_stft
...
class ModelCheckpoint(object):
    def __init__(self, filepath, save_best_only, verbose, min_delta=0.0001):
